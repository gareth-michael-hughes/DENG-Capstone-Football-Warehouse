...
def sql_transform(spark, query, staging_df_1, dbtable_read_base):
...
def main():
    spark = create_spark_session('gwRealtest')
    ...
    dbtable_read_base = 'staging_player_fantasy_stats_gw'
    
    # Outline the query to be executed in the spark job
    query = '''
    SELECT
    CAST(regexp_extract(spfgw.name, '^.+_([^_]+)$', 1) AS INT) as fantasy_id,
    spfgw.name AS fantasy_name,
    CAST(trim(regexp_replace(regexp_replace(spfgw.kickoff_time, 'T', ' '), 'Z', ' ')) as timestamp) as fantasy_kick_off_cleaned,
    CAST(spfgw.total_points as INT) AS player_fixture_fantasy_total_points

    FROM staging_player_fantasy_stats_gw spfgw
    ORDER BY fantasy_kick_off_cleaned ASC, fantasy_name ASC
    '''
    
    # Set the table to write the transformed data to    
    dbtable_write = 'gw_test_fantasy'
    mode = 'append'
    na_replace_val = 'NULL'
    ...
    transform_table = sql_transform(spark, query, staging_df_1, dbtable_read_base)
