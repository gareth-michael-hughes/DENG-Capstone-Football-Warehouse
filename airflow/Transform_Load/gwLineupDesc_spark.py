...
def main():
    spark = create_spark_session('gwLineupDescLoad')
    ...
    dbtable_read_base = 'staging_teams'
    dbtable_read_join_1 = 'staging_fixtures'
    dbtable_read_join_2 = 'staging_lineups_descriptive'
    ...
    home.name AS home_team_name,
     hdesc.formation AS home_team_formation,
     hdesc.coach_id AS home_team_coach_id,
     hdesc.coach AS home_team_coach,
     away.name AS away_team_name,
     adesc.formation AS away_team_formation,
     adesc.coach_id AS away_team_coach_id,
     adesc.coach AS away_team_coach,
     current_timestamp() AS last_data_update
 
     FROM staging_fixtures sf
     LEFT JOIN staging_teams away on away.team_id = sf.awayTeam_team_id
     LEFT JOIN staging_teams home on home.team_id = sf.homeTeam_team_id
     LEFT JOIN staging_lineups_descriptive adesc on away.name = adesc.team_name AND sf.fixture_id = adesc.fixture_id  
     LEFT JOIN staging_lineups_descriptive hdesc on home.name = hdesc.team_name AND sf.fixture_id = hdesc.fixture_id
     ORDER BY sf.fixture_id ASC
            '''
    # Set the table to write the transformed data to    
    dbtable_write = 'dimGameweekLineupDesc'
    mode = 'append'
    na_replace_val = 'NULL'
