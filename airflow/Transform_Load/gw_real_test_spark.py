...
def main():
    ...
    dbtable_read_join_1 = 'staging_team_squads'
    dbtable_read_join_2 = 'staging_fixtures'
    
    # Outline the query to be executed in the spark job
    query = '''
    SELECT

    trim(lower(concat(left(ts.firstname, 1), ts.lastname))) as real_name_cleaned,
    CAST(from_unixtime(sfix.event_timestamp) as timestamp) as real_kick_off_cleaned,
    sfps.player_id,
    sfps.fixture_id
    
    FROM staging_fixture_player_stats sfps
    LEFT JOIN staging_fixtures sfix on sfix.fixture_id = sfps.fixture_id
    LEFT JOIN staging_team_squads ts on sfps.player_id = ts.player_id
    ORDER BY real_kick_off_cleaned ASC, real_name_cleaned ASC
                '''
    
    # Set the table to write the transformed data to    
    dbtable_write = 'gw_test_real'
    mode = 'append'
    na_replace_val = 'NULL'
