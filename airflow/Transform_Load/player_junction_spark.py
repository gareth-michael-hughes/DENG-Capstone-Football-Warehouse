...
def main():
    spark = create_spark_session('player_junction_test')
    ...
    dbtable_read_base = 'staging_lineups_players'
    dbtable_read_join_1 = 'staging_player_fifa_ratings'
    dbtable_read_join_2 = 'staging_player_fantasy_stats_gw'
    
    # Outline the query to be executed in the spark job
    query = '''
    SELECT DISTINCT
    CAST(slp.player_id AS INT) AS player_original_id,
    slp.player AS player_original_name,
    CASE WHEN instr(slp.player, '.') != 0
    THEN trim(lower(replace(replace(slp.player, '.', ''),' ', '')))
    WHEN instr(slp.player, '.') == 0 AND instr(slp.player, ' ') != 0 
    THEN concat(lower(left(trim(slp.player),1)), trim(lower(regexp_extract(replace(trim(slp.player), ' ', '_'), '[^_]*_(.+)', 1))))
    WHEN instr(slp.player, '.') == 0 AND instr(slp.player, ' ') == 0
    THEN trim(lower(slp.player))
    END AS player_original_join_name,
    CAST(fr.Player_id AS INT) AS fifa_id,
    fr.Name AS fifa_name,
    CASE WHEN instr(fr.Name, '.') != 0
    THEN trim(lower(replace(replace(fr.Name, '.', ''), ' ', '')))
    WHEN instr(fr.Name, '.') == 0 AND instr(trim(fr.Name), ' ') == 0
    THEN trim(lower(fr.Name))
    WHEN instr(fr.Name, '.') == 0 AND instr(trim(fr.Name), ' ') != 0
    THEN concat(lower(left(trim(fr.Name),1)), trim(lower(regexp_extract(replace(trim(fr.Name), ' ', '_'), '[^_]*_(.+)', 1))))
    END AS fifa_join_name,
    CAST(regexp_extract(fant.name, '^.+_([^_]+)$', 1) AS INT) AS fantasy_id,
    fant.name AS fantasy_name,
    trim(lower(concat(left(fant.name, 1), regexp_replace(regexp_replace(fant.name, "^[^_]+(?=_)_", ""), "_.*$", "")))) AS fantasy_join_name
    
    FROM staging_lineups_players slp
    LEFT JOIN staging_player_fifa_ratings fr ON 
    CASE WHEN instr(slp.player, '.') != 0
    THEN trim(lower(replace(replace(slp.player, '.', ''),' ', '')))
    WHEN instr(slp.player, '.') == 0 AND instr(slp.player, ' ') != 0 
    THEN concat(lower(left(trim(slp.player),1)), trim(lower(regexp_extract(replace(trim(slp.player), ' ', '_'), '[^_]*_(.+)', 1))))
    ...
    THEN concat(lower(left(trim(fr.Name),1)), trim(lower(regexp_extract(replace(trim(fr.Name), ' ', '_'), '[^_]*_(.+)', 1))))
    END
    LEFT JOIN staging_player_fantasy_stats_gw fant ON
    CASE WHEN instr(slp.player, '.') != 0
    THEN trim(lower(replace(replace(slp.player, '.', ''),' ', '')))
    WHEN instr(slp.player, '.') == 0 AND instr(slp.player, ' ') != 0 
    THEN concat(lower(left(trim(slp.player),1)), trim(lower(regexp_extract(replace(trim(slp.player), ' ', '_'), '[^_]*_(.+)', 1))))
    ...
    END
    =
    trim(lower(concat(left(fant.name, 1), regexp_replace(regexp_replace(fant.name, "^[^_]+(?=_)_", ""), "_.*$", ""))))
    '''
    
    # Set the table to write the transformed data to    
    dbtable_write = 'player_junction_test'
    mode = 'append'
    na_replace_val = 'NULL'
    ...
    transform_table = sql_transform(spark, query, staging_df_1, staging_df_2, staging_df_3, \
                                    dbtable_read_base, dbtable_read_join_1, dbtable_read_join_2)
#     clean_load(transform_table, na_replace_val, dbtable_write, mode, endpoint=url, user=user, password=db_pass)
    
    # convert to a dataframe for easier writing to csv file
    transform_table.toPandas().to_csv("/home/workspace/airflow/Staging/player_mapping.csv")
    print("Junction table data successfully written to csv file")
