...
def main():
    spark = create_spark_session('loadTeamsTable')
    ...
    dbtable_read = 'staging_teams'
    
    # Outline the query to be executed in the spark job    
    query = '''
    SELECT DISTINCT team_id,
    country AS team_country,
    to_date(cast(founded as string), 'YYYY') AS team_founded_date,
    ucase(CAST(is_national as string)) AS is_national_team,
    logo AS team_logo,
    name AS team_name,
    venue_address AS team_venue_address,
    venue_capacity AS team_venue_capacity,
    venue_city AS team_venue_city,
    venue_name AS team_venue_name,
    venue_surface AS team_venue_surface,
    'Premier League 2018-19' AS league_name,
    2 AS league_id,
